...
from com_worktwins_logger.WorkTwinsLogger import WorkTwinsLogger
...
class ProjectContextGenerator:
    ...
    def generate_context_file(self):
        WorkTwinsLogger.log(f"Generating context file: {self.output_file}")
        project_data = {
                ...
                for file in files:
                    ...
                    if (
                        ...
                        try:
                            ...
                            WorkTwinsLogger.log(f"Skipping file {file_path} due to decoding error: {e}")
                        except Exception as e:
                            WorkTwinsLogger.log(f"Skipping file {file_path} due to an unexpected error: {e}")
        ...
        WorkTwinsLogger.log(f"Context file generated at: {self.output_file}")

    def split_file(self, file_path, num_chunks=None, chunk_size=None):
        output_dir = f"{os.path.splitext(file_path)[0]}_parts"
        os.makedirs(output_dir, exist_ok=True)
        WorkTwinsLogger.log(f"Splitting file {file_path} into parts in directory {output_dir}")
        ...
        for i in range(0, len(content), chunk_size):
            ...
            WorkTwinsLogger.log(f"Created part file: {part_filename}")
            part_num += 1
        ...
        WorkTwinsLogger.log(f"Moved original context file to: {new_output_file_path}")
...
def main(
    ...
    if compress:
        ...
        if parts_dir_abs != new_parts_dir_abs:
            ...
            WorkTwinsLogger.log(f"Parts directory moved to: {new_parts_dir}")
        else:
            WorkTwinsLogger.log(f"Parts directory is already in the output folder: {new_parts_dir}")
